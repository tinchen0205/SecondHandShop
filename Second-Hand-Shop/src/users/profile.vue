...
const switchSection = async (section) => {
  ...
  if (section === 'orders') {
    try {
      ...
      const rental_response = await axios.get(`http://localhost:3010/rentalorders/${store.state.userId}`);

      // 設置時間格式選項
      const options = { hour12: false, timeZone: 'Asia/Taipei', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' };

      // 將每個訂單添加 isExpanded 屬性來控制展開狀態，並格式化時間
      orders.value = response.data.orders.map(order => ({
        ...order,
        isExpanded: false,
        orderDate: new Date(order.orderDate).toLocaleString('zh-TW', options),
        tradeDateTime: new Date(order.tradeDateTime).toLocaleString('zh-TW', options)
      }));

      rental_order.value = rental_response.data.orders.map(order => ({
        ...order,
        isExpanded: false,
        orderDate: new Date(order.orderDate).toLocaleString('zh-TW', options),
        tradeDateTime: new Date(order.tradeDateTime).toLocaleString('zh-TW', options),
        returnDateTime: new Date(order.returnDateTime).toLocaleString('zh-TW', options)
      }));
    } catch (error) {
...
const togglerentalOrder = (index) => {
  if (rental_order.value[index]) {
    rental_order.value[index].isExpanded = !rental_order.value[index].isExpanded;
  }
};
...
<table class="table table-bordered">
  ...
  <tbody>
    <!-- 使用 v-for 遍歷租借訂單資料 -->
    <template v-for="(order, index) in rental_order" :key="order.orderId">
      <tr>
        <td>{{ order.orderId }}</td>
        <td>{{ order.orderDate }}</td>
        <td>{{ order.totalAmount }}</td>
        <td>
          <button class="btn btn-link" @click="togglerentalOrder(index)">
            {{ order.isExpanded ? '收起' : '查看' }}
          </button>
        </td>
      </tr>
